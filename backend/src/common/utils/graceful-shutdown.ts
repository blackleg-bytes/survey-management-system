...
export function setupGracefulShutdown(app: INestApplication, logger: AppLogger): void {
  ...
  const shutdown = async (signal: string) => {
    ...
    logger.warn(`Received ${signal}, starting graceful shutdown...`, 'SHUTDOWN');

    // ⏱ Force-exit timeout (starts when shutdown begins)
    const shutdownTimeout = setTimeout(() => {
      logger.error('Graceful shutdown timeout exceeded — forcing exit', undefined, 'SHUTDOWN');
      process.exit(1);
    }, 30_000);
    ...
    try {
      ...
      try {
        ...
        if (err instanceof Error) {
          ...
          logger.error('Unknown error closing database connections', undefined, 'SHUTDOWN');
        }
      }
      ...
      try {
        ...
        if (redis?.status === 'ready') {
          await redis.quit();
          logger.log('Redis connections closed', 'SHUTDOWN');
        }
      } catch (err: unknown) {
        if (err instanceof Error) {
          logger.error(err.message, err.stack, 'SHUTDOWN');
        } else {
          logger.error('Unknown error closing Redis connections', undefined, 'SHUTDOWN');
        }
      }
  ...
  process.on('unhandledRejection', (reason: unknown) => {
    if (reason instanceof Error) {
      ...
      logger.error(`Unhandled Rejection: ${JSON.stringify(reason)}`, undefined, 'PROCESS');
    }
